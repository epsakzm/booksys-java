...
public class StaffUI extends Canvas implements BookingObserver 
{
  ...
  private int timeToX_end(Time etime){
      int res = LEFT_MARGIN
              + PPH * (etime.getHours() - TZERO)
              + PPM * etime.getMinutes();
    return res;
  }
...
//
      ...
      int ex = timeToX_end(b.getEndTime());
      int y = tableToY(b.getTable().getNumber()) ;
      og.setColor(Color.gray) ;
      //보이는 회색
      og.fillRect(x, y, ex-x, ROW_HEIGHT) ;
      if (b == bs.getSelectedBooking()) {
	og.setColor(Color.red) ;
	//보이는 빨강색
	og.drawRect(x, y, ex-x, ROW_HEIGHT) ;
      }
    ...
    if (mouseDown && b != null) {
      ...
      int ex = timeToX_end(b.getEndTime()) + currentX - firstX;
  ...
  void addReservation()
  {
    ...
    String resLog = "";
    if (d.isConfirmed()) {
      resLog = bs.makeReservation(d.getCovers(),
			 bs.getCurrentDate(),
			 d.getTime(),
			 d.getEndTime(),
			 d.getTableNumber(),
  ...
  void addWalkIn()
  {
    ...
    String walkinLog = "";
    if (d.isConfirmed()) {
      walkinLog = bs.makeWalkIn(d.getCovers(),
		    bs.getCurrentDate(),
		    d.getTime(),
		    d.getEndTime(),
		    d.getTableNumber()) ;
    }
    log("Walk-In added" + walkinLog);
  }
  
  void cancel()
  {
    log("Booking Canceled\n"+bs.getSelectedBooking().getMoreDetails());
    bs.cancel() ;
  }
  ...
  void logOut(){
    log(CurrentState.getInstance().getAdmin().getId() + " Logged Out");
    bs.logOut();
  }

  private void log(String msg){
    Recorder.record().log(msg);
  }
}
