...
public class BookingMapper
{
  ...
  public Vector getBookings(Date date)
  {
	...
	Time etime = rset.getTime(5);
	int table = rset.getInt(6) ;
	...
	PersistentReservation r
	  = new PersistentReservation(oid, covers, bdate, btime, etime, t, c, atime) ;
	v.add(r) ;
      }
      rset.close() ;
	...
	PersistentTable t = TableMapper.getInstance().getTableForOid(table) ;
	PersistentWalkIn w
	  = new PersistentWalkIn(oid, covers, bdate, btime, etime, t) ;
	v.add(w) ;
      }
      rset.close() ;
     stmt.close() ;
				       ...
				       Table table)
  {
    ...
    performUpdate("INSERT INTO WalkIn " + "VALUES ('"
		  ...
		  + time + "', '"
            + etime + "', '"
		  + ((PersistentTable) table).getId() + "')" ) ;
    return new PersistentWalkIn(oid, covers, date, time, etime, table) ;
  }
